var createWorldCreator = function() {
    ...
    creator.createWorld = function(options) {
        ...
        var recalculateStats = function() {
            world.transportedPerSec =
                world.transportedCounter / world.elapsedTime;
        ...
        var handleElevAvailability = function(elevator) {
            ...
            for (
                var users = world.users, i = 0, len = users.length;
                i < len;
                ++i
            ) {
                var user = users[i];
                if (user.currentFloor === elevator.currentFloor) {
                    user.elevatorAvailable(
                        elevator,
                        world.floors[elevator.currentFloor],
                    );
                }
            }
        };
        ...
        var handleButtonRepressing = function(eventName, floor) {
            ...
            for (
                var i = 0,
                    len = world.elevators.length,
                    offset = _.random(len - 1);
                i < len;
                ++i
            ) {
                ...
                if (
                    (eventName === 'up_button_pressed' &&
                        elevator.goingUpIndicator) ||
                    (eventName === 'down_button_pressed' &&
                    ...
                    if (
                        ...
                        world.elevatorInterfaces[elevIndex].goToFloor(
                            floor.level,
                            true,
                        );
                        return;
        ...
        world.update = function(dt) {
            ...
            for (
                var users = world.users, i = world.users.length - 1;
                i >= 0;
                i--
            ) {
                var u = users[i];
