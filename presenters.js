...
function presentStats($parent, world) {
    ...
    world.on('stats_display_changed', function updateStats() {
        ...
        elem_transportedpersec.textContent = world.transportedPerSec.toPrecision(
            3,
        );
...
function presentWorld(
    ...
    function setUpElevator(e) {
        ...
        $elevator
            .find('.buttonindicator')
            .html(renderElevatorButtons(e.buttonStates));
        var $buttons = _.map(
            $elevator.find('.buttonindicator').children(),
            function(c) {
                return $(c);
            },
        );
        var elem_floorindicator = $elevator
            .find('.floorindicator > span')
            .get(0);
        ...
        e.on('floor_buttons_changed', function update_floor_buttons(
            states,
            indexChanged,
        ) {
            $buttons[indexChanged].toggleClass(
                'activated',
                states[indexChanged],
            );
        });
        e.on('indicatorstate_change', function indicatorstate_change(
            indicatorStates,
        ) {
            $elevator.find('.up').toggleClass('activated', indicatorStates.up);
            $elevator
                .find('.down')
                .toggleClass('activated', indicatorStates.down);
        });
...
function makeDemoFullscreen() {
    ...
    $('html, body, body .container, .world').css({
        width: '100%',
        margin: '0',
        padding: 0,
    });
