...
var newGuard = function(obj, type) {
    if (!(obj instanceof type)) {
        throw 'Incorrect instantiation, got ' +
            typeof obj +
            ' but expected ' +
            type;
    }
...
distanceNeededToAchieveSpeed = function(
    currentSpeed,
    targetSpeed,
    acceleration,
) {
    ...
    var requiredDistance =
        (Math.pow(targetSpeed, 2) - Math.pow(currentSpeed, 2)) /
        (2 * acceleration);
    return requiredDistance;
};
accelerationNeededToAchieveChangeDistance = function(
    currentSpeed,
    targetSpeed,
    distance,
) {
    ...
    var requiredAcceleration =
        0.5 *
        ((Math.pow(targetSpeed, 2) - Math.pow(currentSpeed, 2)) / distance);
